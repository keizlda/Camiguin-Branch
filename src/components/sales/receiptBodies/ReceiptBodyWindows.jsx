...
// flex-wrap lets a value drop to its own line instead of running off the
// page when it doesn't fit next to the label; min-w-0 overrides flex's
// default min-width:auto, which otherwise blocks a flex child from
// shrinking/wrapping even when its sibling needs the room.
...
// Receipt body for printing from a Windows PC. Starts out identical to
// ReceiptBodyMac.jsx (Windows Chrome renders real CSS same as macOS
// Chrome does, and this shop hasn't hit a Windows-specific print issue
// yet) but is deliberately its own separate component, not a shared one
// with a platform flag — if/when a real Windows print turns up its own
// quirk (a different thermal driver stack than macOS's), it can be
// tuned here without touching Mac's or Android's files. See
// PrintReceiptModal.jsx and ReceiptBodyAndroid.jsx for the real,
// print-tested reasons those two differ from this one.
function ReceiptBodyWindows({ receipt }) {
...
export default ReceiptBodyWindows;
